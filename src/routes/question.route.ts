...
const questionRouter = (jwtSecret: string) => {
  ...
  router.post(
    ...
    async (req: express.Request<ParamsDictionary, never, IQuestionWithId>, res: express.Response) => {
      try {
        ...
        const { id: iD, text, type, answers, variable, scored } = req.body;

        const question: Nullable<DocumentQuestion> = await Question.findOne({ _id: iD });
