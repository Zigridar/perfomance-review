...
const formRouter: (jwtSecret: string) => Router = (jwtSecret: string) => {
  ...
  router.put(
    ...
    async (req: express.Request<ParamsDictionary, never, IForm>, res: express.Response) => {
      try {
  ...
  router.get(
    ...
    async (req: express.Request<never, never, never, FormQuery>, res: express.Response) => {
      try {
  ...
  router.post(
    ...
    async (req: express.Request<ParamsDictionary, never, IFormWithId>, res: express.Response) => {
      try {
        ...
        const { id: iD, description, type, questions, author, archived } = req.body;

        /** find form by id */
        const form: Nullable<DocumentForm> = await Form.findOne({ _id: iD });
